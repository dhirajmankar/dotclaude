...
- [2026-07-22] distillation: promoted from 3 independent sources (gstack pitfall freemium-gate-isPaid-vs-isReadOnly 2026-06-04, learnings.md 2026-06-14, learnings.md ui-bug-batch 2026-07-21) — clarifies Pattern 6's `getIsReadOnly()` guard: the client-side gate and the server-side RLS policy (`has_active_subscription()`) must derive trial expiry from the SAME column. One incident used `isReadOnly` vs `isPaid` confusion; a later one had the client computing trial expiry from `trial_started_at` via IST-day arithmetic while RLS used `trial_ends_at` via exact-timestamp comparison — they usually agreed (seeded as `trial_started_at + 14d`) but drifted near the boundary, letting a button render while the DB silently rejected the write. Whenever a client permission gate and a server RLS policy both answer "is this allowed" from independently-derived logic, that's a drift bug waiting to happen — one must defer to the other's source column, never re-derive separately.
